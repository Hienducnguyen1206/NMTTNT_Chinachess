...
// Kiểm tra và lấy danh sách nước đi hợp lệ của từng quân cờ
public class MoveCheck : MonoBehaviour
{
    private Board boardInstance;
    private static List<Vector2Int> MoveablePoints;
    private static List<Vector2Int> PreMoveablePoints;
    ...
    void Start()
    {
        ...
        BluechessPieces = boardInstance.GetBlueChessList();
        RedchessPieces = boardInstance.GetRedChessList();
  ...
  private  Vector2Int CheckChessPosition(string[][][] chessboard, string color, string piece)
    ...
    // Kiểm tra và trả về danh sách nước đi của quân cờ được chọn
    public void AccessSelection()
    {
        Transform selectedObject = SelectChess.GetSelection();

        if (selectedObject != null)
        {
                string[] key = selectedObject.name.Split('_');
                string color = key[0];
                string piece = key[1];
                string type = piece.Substring(0, piece.Length - 1);
         
                switch (piece)
                {
                    case "Xe1":
                        position = CheckChessPosition(board, color, piece);
                        PreMoveablePoints = FindMoveablePoints(position, color, type,board);
                        MoveablePoints = CheckMoveablePoints(PreMoveablePoints, color, piece, position);                                                                  
                        break;
                    case "Xe2":
                        position = CheckChessPosition(board, color, piece);
                        PreMoveablePoints = FindMoveablePoints(position, color, type, board);
    ...
    public List<Vector2Int> GetMoveablePoints()
    {
        return MoveablePoints;
    }

    // Kiểm tra nước các nước đi hợp lệ
    ...
    public List<Vector2Int> CheckMoveablePoints(List<Vector2Int> PreMoveablePoints, string color, string piece, Vector2Int position)
    {
